...
class Comment {
    static async getByTask(taskId) {
        const { rows } = await pool.query(`SELECT c.id, c.text, c.created_at, c.user_id, u.username
       FROM comments c
    ...
    static async create({ userId, taskId, text }) {
        const { rows } = await pool.query(`INSERT INTO comments (user_id, task_id, text)
       VALUES ($1, $2, $3)
       RETURNING *`, [userId, taskId, text]);
        return rows[0];
    }
